import Link from "next/link";
import { BrandIcon } from "@/components/icons/BrandIcon";

export function Footer() {
    return (
        <footer className="border-t border-border/40 bg-background/50 backdrop-blur-xl mt-auto" role="contentinfo">
            <div className="mx-auto max-w-7xl px-6 py-12 md:flex md:items-center md:justify-between lg:px-8">
                <nav aria-label="Footer links" className="flex justify-center space-x-6 md:order-2">
                    <Link
                        href="https://github.com/kneeraazon404/ttstt"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-muted-foreground hover:text-primary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
                        aria-label="View source code on GitHub (opens in new tab)"
                    >
                        <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                            <path
                                fillRule="evenodd"
                                d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
                                clipRule="evenodd"
                            />
                        </svg>
                    </Link>
                </nav>
                <div className="mt-8 md:order-1 md:mt-0">
                    <Link
                        href="/"
                        className="inline-flex items-center gap-2 justify-center md:justify-start mb-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
                        aria-label="VoiceTech 2026 — Home"
                    >
                        <div className="rounded-full overflow-hidden shrink-0">
                            <BrandIcon size={24} />
                        </div>
                        <span className="font-display text-sm font-bold text-foreground/70">VoiceTech 2026</span>
                    </Link>
                    <p className="text-center md:text-left text-xs leading-5 text-muted-foreground">
                        &copy; {new Date().getFullYear()} Voice Tech Intelligence. Built by{" "}
                        <Link
                            href="https://kneeraazon.com"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
                        >
                            kneeraazon.com
                        </Link>
                        . Data from Artificial Analysis, HuggingFace TTS Arena &amp; official provider docs.
                    </p>
                </div>
            </div>
        </footer>
    );
}
